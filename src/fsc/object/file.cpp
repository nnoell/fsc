...
File::File(std::filesystem::directory_entry entry, base::TransformData transform_data, glm::mat4 model) :
    base::Complex(
      {
        // The cube
        std::make_shared<base::Polygon>(base::vertices::GetCube(), entry.is_directory() ? glm::vec4 {0.0f, 1.0f, 1.0f, 1.0f} : ( entry.is_regular_file() ? glm::vec4 {0.2f, 0.6f, 1.0f, 1.0f} : glm::vec4 {0.3f, 0.5f, 1.0f, 1.0f}), false),
        // The file name label
